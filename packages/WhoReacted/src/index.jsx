...
import { Settings, Utilities } from '@zlibrary/api';
import Plugin from '@zlibrary/plugin';

import Reactors from './components/Reactors';
import style from './style.scss';

const { Webpack, UI: { showToast }, Patcher, DOM, ReactUtils } = BdApi;

const Reaction = Webpack.getModule(m => m.type?.toString().includes('isBurstReaction'), { searchExports: true });
...
export default class WhoReacted extends Plugin {
                ...
                new Textbox(
                    ...
                    value => {
                        if (isNaN(value) || value < 1 || value > 99) {
                            return showToast('Value must be a number between 1 and 99!', { type: "error" });
                        }
    ...
    patchReaction() {
        let patched = false;
        Patcher.after(Reaction, 'type', (_, __, outerReturnValue) => {
            if (patched) return;
            patched = true;

            Patcher.after(outerReturnValue.type.prototype, 'render', (thisObject, args, returnValue) => {
    ...
    findReactionReactInstance(node) {
        return Utilities.findInTree(ReactUtils.getInternalInstance(node), r => r?.type?.displayName === 'Reaction', {
            walkable: ['return']
        });
    }
}
